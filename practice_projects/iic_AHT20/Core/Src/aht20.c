...
//读取函数，温度为temperature,湿度为humidity
void AHT20_Read(float *humidity, float *temperature)
{
    uint8_t readbuffer[6];//用于存放发过来的6个字节数据
    uint8_t sendBuffer[3] = {0xAC,0x33,0x00};//开始测量指令
    //发送函数，地址与开始测量指令均发送
    HAL_I2C_Master_Transmit(&hi2c1, AHT20_ADDRESS, sendBuffer, 3,HAL_MAX_DELAY);
    HAL_Delay(75);
    //读取发回来的的6字节数据，存在readbuffer
    HAL_I2C_Master_Receive(&hi2c1, AHT20_ADDRESS, readbuffer, 6,HAL_MAX_DELAY);
    //检测第0字节的第7位是否为0，0（0x00）正常，1（0x80）报错
