...
/**
  ...
  * @retval HAL status
  */
static HAL_StatusTypeDef RTC_EnterInitMode(RTC_HandleTypeDef *hrtc)//进入RTC初始化模式
{
...
/**
  ...
  * @retval HAL status
  */
static HAL_StatusTypeDef RTC_ExitInitMode(RTC_HandleTypeDef *hrtc)//退出RTC初始化模式
{
  uint32_t tickstart = 0U;
...
/**
  ...
  * @retval Time counter
  */
static uint32_t RTC_ReadTimeCounter(RTC_HandleTypeDef *hrtc)//读取RTC计数器的值
{
...
/**
  ...
  * @retval HAL status
  */
static HAL_StatusTypeDef RTC_WriteTimeCounter(RTC_HandleTypeDef *hrtc, uint32_t TimeCounter)//写入RTC计数器的值
{
...
{
  uint32_t unixtime = mktime(zj_time);
